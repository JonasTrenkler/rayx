...
if ($CUDA_VERSION_FULL -eq "13.3.0") {
    $downloadUrl = "https://developer.download.nvidia.com/compute/cuda/13.3.0/local_installers/cuda_13.3.0_windows.exe"
} elseif ($CUDA_VERSION_FULL -eq "12.9.1") {
...
} elseif ($CUDA_VERSION_FULL -eq "12.3.2") {
    $downloadUrl = "https://developer.download.nvidia.com/compute/cuda/12.3.2/local_installers/cuda_12.3.2_546.12_windows.exe"
...
(Get-ChildItem 'C:\Program Files\Microsoft Visual Studio\18\*\MSBuild\Microsoft\VC\*\BuildCustomizations').FullName | ForEach-Object {
    $destination = $_
